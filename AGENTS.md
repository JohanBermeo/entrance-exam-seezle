...
- **No PR or merge by default.** Do not create pull requests, run `gh pr create`, or merge into `main` unless the user explicitly requests it in that conversation.
- **End of milestone:** run verification; update the matching README (`backend/README.md` or `frontend/front-calculator/README.md`, or both if applicable); give a short summary and branch name, then **stop and wait** for user approval before any new scope.
