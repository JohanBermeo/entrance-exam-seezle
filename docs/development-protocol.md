...
El agente **no** debe usar `gh pr create`, push con intención de abrir PR, ni mergear ramas salvo que el humano lo pida **explícitamente** en ese momento.
...
6. **Entrega del hito (agente):** resumir qué quedó hecho, cómo verificarlo, qué README se actualizó y en qué rama está el trabajo; **detenerse y esperar**.
7. **Aprobación del hito (humano):** cuando el humano da por aprobado el hito, decide si abre PR, cuándo mergea y si continúa al siguiente hito.
...
- Ramas cortas que nacen de `main` y vuelven con PR pequeño (gestionado por el humano).
